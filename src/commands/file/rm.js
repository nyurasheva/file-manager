...
export async function rm(filepath) {
  try {
    if (!filepath) {
      console.error(
        `${INVALID_CMD_MESSAGE_MAGENTA}${FG_MAGENTA} (rm command should have a parameter, e.g. "rm path_to_file")${RESET}`
      );
      printCwd();
      return;
    }
    ...
    if (stats.isDirectory()) {
      console.error(
        `${INVALID_CMD_MESSAGE_MAGENTA}${FG_MAGENTA}. Cannot delete directory using rm.${RESET}`
      );
      printCwd();
      return;
    }
    ...
    console.log(
      `${FG_GREEN}${BRIGHT}${filepath}${RESET}${FG_GREEN} has been deleted.${RESET}`
    );
    printCwd();
  } catch (err) {
    if (err.code === 'ENOENT') {
