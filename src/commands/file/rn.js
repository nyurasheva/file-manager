...
export async function rn(filepath, newName) {
  try {
    if (!filepath || !newName) {
      console.error(`${INVALID_CMD_MESSAGE_MAGENTA}${FG_MAGENTA} (rn command should have two parameters, e.g. "rn path_to_file new_filename")${RESET}`);
    ...
    console.log(`${FG_GREEN}${filepath} renamed to ${BRIGHT}${newName}${RESET}`);
    printCwd();
  } catch (err) {
